...
# ============================================================================
# CONFIGURATION (V9.6 - Optimized V9.4 for speed: reduced history, conditional flow, frame skipping)
# ============================================================================
DEFAULT_GARBAGE_MODEL = '/root/IWDD2026/models/yolo_garbage_colab_v5.pt'
CONF_USE = 0.15  # Kept low for recall
IOU_THRESHOLD = 0.5
IMG_SIZE_MAIN = 1280
...
# Optical flow (conditional, more sensitive)
FLOW_MAG_THRESH = 1.0
FLOW_WIN_SEC = 0.5
FLOW_COND_PATH_THRESH = 20  # Compute flow only if path_len < this

# Dual BG params (reduced history for speed)
SHORT_HISTORY = 50  # Reduced
SHORT_VAR_THRESH = 25
LONG_HISTORY = 1000  # Reduced
LONG_VAR_THRESH = 16
DF_RATIO_THRESH = 0.3
FG_RATIO_THRESH = 0.2

# Frame skipping for CV ops
SKIP_FRAMES = 2  # Process BG/flow every N frames

# Debug
DEBUG_ENABLED = True
DEBUG_SUBDIR_NAME = 'debug_v9_6'

# ============================================================================
# ARGUMENTS
# ============================================================================
def parse_arguments():
    p = argparse.ArgumentParser(description='IWDD v9.6 Inference - Optimized V9.4 for speed')
...
def process_single_video(video_path, output_dir, model_garbage, debug=False):
    ...
    # Init dual background subtractors and optical flow
    ...
    if debug:
        dbg_lines += [
            ...
            f"conf_use={CONF_USE:.2f} flow_mag_thresh={FLOW_MAG_THRESH:.1f} df_ratio_thresh={DF_RATIO_THRESH:.1f}",
            f"skip_frames={SKIP_FRAMES} flow_cond_thresh={flow_cond_thresh:.1f}",
            ""
        ]
    ...
    dbg_accepts_dual_bg = 0

    final_timestamp = None
    frame_count = 0
    cv_count = 0  # For skipping
    ...
    for res in results_stream:
        ...
        # Skip BG/flow every SKIP_FRAMES
        ...
        for box, tid, det_conf in zip(boxes, tids, confs):
            ...
            if do_cv:
                ...
                df_ratio = np.sum(roi_df) / (roi_df.size + 1e-9) if roi_df.size > 0 else 0.0
            ...
            # Conditional optical flow (only if do_cv and low path)
            ...
            if (not st['was_moving']) and (not near_cluster) and (st['path_len'] <= soft_path_px) and (not eligible_by_area_jump) and (not eligible_by_flow) and (not is_foreground) and (not eligible_by_dual_bg):
            ...
            # Novelty ok
            novelty_ok = (eligible_by_soft_path or eligible_by_area_jump or near_cluster or eligible_by_flow or is_foreground or eligible_by_dual_bg)
            ...
            if eligible:
                ...
                if stationary_time >= (STATIONARY_DUR + PERSIST_AFTER_SEC):
                    ...
                    if eligible_by_new and not eligible_by_move:
                        ...
                        if eligible_by_dual_bg:
                            dbg_accepts_dual_bg += 1
    ...
    if debug:
        ...
        dbg_lines.append(f"accepts_optical_flow={dbg_accepts_optical_flow} accepts_dual_bg={dbg_accepts_dual_bg}")
...
def main():
    ...
    print("=== IWDD v9.6 Inference (optimized V9.4 for speed) ===")
