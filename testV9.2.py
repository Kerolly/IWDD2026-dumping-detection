...
# ============================================================================
# CONFIGURATION (V9+ novelty/cluster/persistence)
# ============================================================================
...
# Near-cluster (new item near existing pile)
CLUSTER_RADIUS_BASE = 240   # px on diag~1500
CLUSTER_MIN_AGE_SEC = 0.8

# Long-term static background + persistence after confirm
...
# ============================================================================
def parse_arguments():
    p = argparse.ArgumentParser(description='IWDD v9+ Inference - new/moving + novelty/cluster gates')
    ...
    for res in results_stream:
        ...
        for box, tid, det_conf in zip(boxes, tids, confs):
            ...
            if st['seen_in_init'] and not st['bg_checked'] and current_time >= (START_CHECK_WINDOW + 1.0 / fps):
                ...
                if disp_bg < BG_MOVEMENT_TOL_BASE_PX * scale:
            ...
            if (not st['was_moving']) and (not near_cluster) and (st['path_len'] <= soft_path_px) and (not eligible_by_area_jump):
            ...
            # New acceptance rule (must have novelty if appeared_after_init)
            novelty_ok = (eligible_by_soft_path or eligible_by_area_jump or near_cluster)
...
def main():
    ...
    print("=== IWDD v9+ Inference (new/moving + novelty/cluster) ===")
