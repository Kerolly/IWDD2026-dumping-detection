...
DEFAULT_GARBAGE_MODEL = 'models/yolo_garbage_colab_v4.pt'
    ...
    for res in results_stream:
        ...
        if res.boxes is None or res.boxes.id is None or len(res.boxes) == 0:
            ...
            for box, tid, det_conf in zip(boxes, tids, confs):
                ...
                # Accumulate path length (smooth)
                if len(st['positions_smooth']) >= 2:
                    p1 = st['positions_smooth'][-2]
                    p2 = st['positions_smooth'][-1]
                ...
                # Eligibility: moving OR new-appearance OR soft-path
                eligible_by_move = st['was_moving']
                eligible_by_new  = st['appeared_after_init']
                eligible_by_soft_path = (st['path_len'] > soft_path_px)

                eligible = (
                    is_stationary_now and (not agent_near) and old_enough and
                    (eligible_by_move or eligible_by_new or eligible_by_soft_path)
                )
